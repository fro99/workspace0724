...
public class Variable {
	...
	public static void main(String[] args) {
		...
		float num2;
		num2 = 3.141234546431f;
		System.out.printf("%.6f", num2);
		
		//----------------------------------------------------------
		//원시타임 : 가장 기본적인 데이터타입, 값 자체를 저장하며 추가적인 속성이 없다.
		 ...
		 * 문자열
		 * String
		 * 문자열 같은 경우 할당할 메모리크기가 가변적이므로 원시타입이 아닌 객체타입으로 변수륾 만들어 사용함.
		 * 
		 * 원래 객체를 만드는 방식
		...
		/*
		 * 이스케이프 시퀀스
		 * 문자열내에서 탭, 백슬래시, 작은따옴표 등을 사용하기 위한 방식
		 */
...
//		MY_AGE = 111; --값을 한번 초기화하면 더이상 변경할 수 없음.
		...
		/*
		 * 프로그래밍에서 이름 짓는 방식
		 * 카멜케이스 : 단어를 나열하며 이름을 짓되, 두번째단어부터는 단어의 첫 글자를 대문자로 시작(자바에서 모든 메서드, 변수명)
		 *     ex) userName, totalPrice
		 *
		 * 파스칼케이스 : 카멜케이스에서 첫 글자도 대문자로 작성(자바에서는 클래스명에 사용)
		 *     ex) UserName, TotalPrice
		 *
		 * 스네이크케이스 : 문자와 문자의 사이를 언더스코어(_)를 통해서 이어주는 것(파이썬에서 주로 사용)
		 *     ex) user_name, total_price
		 *
		 * 대문자스네이크케이스 : 스네이크케이스 + 모두 대문자 (대부분 언어에서 상수명)
		 *     ex) USER_NAME
		 *
		 * 케밥케이스 : 단어를 소문자로 작성하며 문자와 문자 사이를 하이픈(-)으로 이어주는 것(html속성, url, 설정파일 등)
		 *     ex) html-속성, url-경로
		 */
	}
