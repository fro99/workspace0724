/*
    localStorage
    -브라우저에 key-value형대로 데이터를 저장할 수 있는 공간
...
function addTodo(){
    ...
    const todo = {
        id: Date.now(), //현재시간을 ms단위로 변환 -> 교유한 ID로 사용
        content: text,
        completed: false,
...
function toggleTodo(id){
    //해당 id를 통해서 할일을 찾아 완료상태면 -> 미완료, 미완료 -> 완료 변경.
    for(let todo of todos){
        if(todo.id ===id){
...
//메일 렌더링 함수
function render(){
    todoList.innerHTML = ""; //기존 UI제거

    //현재 필터에 맞는 할일만 목록으로 가져오기
    const filteredTodos = getFilteredTodos();

    if(filteredTodos.length === 0){ //할일 목록이 비어있다면
        emptyStateRender();
    } else{ //할일 목록이 있는 경우
...
function todoItemRender(todo){
    ...
    //새로 생성된 요소들 중에서 이벤트가 필요한 부분만 가져외.
...
//DOMContentLoaded -> HTML이 전부 로드외어 DOM트리가 완성되면 실행
