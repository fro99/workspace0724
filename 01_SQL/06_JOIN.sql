...
--1. 연결한 두 컬럼명이 다른 경우 JOIN IN을 사용해서 연결할 컬럼명을 명시적으로 연결
SELECT EMP_ID, EMP_NAME, DEPT_CODE, DEPT_TITLE
FROM EMPLOYEE
...
/*
    ...
    단, 반드시 LEFT/RIGHT를 지정해야된가.(어떤 테이블을 기준으로 할것인가)
*/
--외부조인과 비교할만한 내부조인
--사원명, 부서명, 급여, 연봉
...
--FULL [OUTER] JOIN : 두테이블이 가진 모든행을 조회할 수 있다.
SELECT EMP_NAME, DEPT_TITLE, SALARY, SALARY*12
FROM EMPLOYEE
...
/*
    ...
    동일한 컬럼명이 각 테이블에 존재하는 경우에만 정산적인 조인이 가능하다.
*/
SELECT *
FROM EMPLOYEE
NATURAL JOIN JOB;