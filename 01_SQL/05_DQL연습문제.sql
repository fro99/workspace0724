...
SELECT EMP_NAME, SALARY*12, (salary + (salary*nvl(bonus, 0)))*12 ,(salary + (salary*nvl(bonus, 0)))*12-(salary*12*0.03)
FROM EMPLOYEE;
...
--17. EMPLOYEE 테이블에서 사원명, 입사일-오늘, 오늘-입사일 조회 (단, 각 별칭은 근무일수1, 근무일수2가 되도록 하고 모두 정수(내림), 양수가 되도록 처리)
...
SELECT *
FROM EMPLOYEE
WHERE CEIL(SYSDATE-HIRE_DATE)>=20;
...
--22. EMPLOYEE 테이블에서 부서코드가 D5, D6, D9인 사원만 조회하되 D5면 총무부, D6면 기획부, D9면 영업부로 처리 (단, 부서코드 오름차순으로 정렬)
SELECT *
FROM EMPLOYEE
...
--25. EMPLOYEE 테이블에서 직원들의 입사일로부터 년도만 가지고 각 년도별 입사 인원수 조회 (전체 직원 수, 2001년, 2002년, 2003년, 2004년)
SELECT COUNT(*)
FROM EMPLOYEE
WHERE 
