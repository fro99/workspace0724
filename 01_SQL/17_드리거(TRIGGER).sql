/*
    <트리거>
    특정 테이블에 대해 INSERT, UPDATE, DELETE외 같은 DML이벤트가 발생했을 때
    ...
    *적용대상(어디에 적용하는가?)
    -문장 트리거: 이벤트가 발생한 SQL문에 대해 딱 한번만 트리거 실행(FOR EACH ROW 없음)
...
-- 입출고에 대한 데이터 기록(INSERT)될 때마다 해당 상품에 대한 재고수량을 매번 수정(UPDATE)
-- 상품 테이블(재고), 입출고 기록 테이블
...
INSERT INTO TB_PRODUCT VALUES(SEQ_PCODE.NEXTVAL, '대륙폰', '샤오미', 990000, DEFAULT);

-- 2. 상품 입출고 상세 이력 테이블 생성 (TB_PRODETAIL)
-- 어떤 상품이 어떤 날짜에 몇 개가 입/출고 되는지 기록
CREATE TABLE TB_PRODETAIL (
    ...
    STATUS   CHAR(3) CHECK(STATUS IN ('IN', 'OUT'))       -- 입출/출고 여부
);
...
-- TB_PRODUCT 테이블에 매번 자동으로 재고수량을 UPDATE하는 트리거
