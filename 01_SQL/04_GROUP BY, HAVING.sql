/*
    ...
    여러개의 값들을 하나의 그룹으로 묶어서 처리하는 목정으로 사용
*/
...
--GROUP BY절에 함수식 사용가능(그룹을 나눌 수 있는 기분만 넣어주면)
--남자사원과 여자사원의 수를 조회
...
--WHERE절이 GROUP BY이전에 실행되므로 해당 코드는 300만원 이상의 급여를 받는 사람을 먼저 필터링한 수
--부서코드와 평균을 구한다.
SELECT DEPT_CODE, ROUND(AVG(SALARY))
FROM EMPLOYEE
WHERE SALARY >=3000
...
--직급별 직급코드, 총 급여함(단, 직급별 총급여합이 1000만원 이상인 직원만 조회)
