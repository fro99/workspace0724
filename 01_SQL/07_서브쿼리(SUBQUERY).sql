...
/*
    ...
    비교대상 >, < ANY(값~): 여러개의 결과값 중 한개라고 비교연산을 만족하는가?
    비교대상 >, < ALL(값~): 여러개의 결과값 모두가 비교연산을 만족하는가?
    위 3가지 연산자와 함께 사용하는 경우가 많음.
*/

--1) 우재식 또는 윤은해 사원과 같은 직급인 사원들의 사번, 사원명, 직급코드, 급여조회
SELECT EMP_ID, EMP_NAME, JOB_CODE, SALARY
FROM EMPLOYEE
WHERE JOB_CODE = 'J3' OR JOB_CODE = 'J7';
...
--2) 대리직급임에도 과장직급의 최소급여보다 더 많이 받는 사원들의 사번, 이름, 직급, 급여
...
/*
    3. 다중열 서브쿼리
    결과값이 한 행이지만 나열된 컬럼수가 여러개일 경우(두개 이상의 컬럼)
*/
...
SELECT EMP_ID, EMP_NAME,
      (SALARY + (SALARY * NVL(BONUS, 0))) * 12 AS "연봉", DEPT_CODE
FROM EMPLOYEE
...
--ROWNUM : 오라클에서 기본적으로 제공해주는 컬럼, 조회된 수서대로 1부터 순번을 부여해줍니다.
--         단, ROWNUM은 이미 FROM절에서 결졍이됨.
-->인라이뷰를 통해서 TOP-N분석 : 상위 몇개만 조회
--전 직원 중 급여가 가장 높은 직원 5명
SELECT ROWNUM,EMP_NAME, SALARY
FROM EMPLOYEE
ORDER BY SALARY DESC;

--> ORDER BY절이 수행된 결과를 가지고 ROWNUM부여
SELECT ROWNUM, EMP_NAME, SALARY
