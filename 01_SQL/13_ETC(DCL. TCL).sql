/*
    <DCL : 데이터 제어문>
    계정에 시스템권한 또는 객체접근 권한을 보여하거나 회수하는 구문.
    ...
    ORACLE은 기본적으로 사용자 생성 후 권한을 부여해야 로그인 및 객체사용 가능
*/
...
/*
    <TCL : 트랜잭션 제어문>
    트랜잭션 하나이상의 DML(INSERT, UPDATE, DELETE)문을 묶어서 하나의 논리적 작업 단위로 처리하는 개념.
    ...
    SAVEPOINT : 롤백용으로 중간지접을 지정
    ROLLBACK TO : 특정 지점까지 복원
    
    대부분 요즘은 AUTOCOMMIT이 설정되어 있어서 DML을 전달할 때마다 자동으로 커밋이 됨.
*/
...
/*
    ...
    ACID속성
    트랜잭션의 일관성과 신뢰성을 보장하기위해 지켜야하는 4가지 핵심 속성
    
    A(Automicity) 원자성 : 트랜잭션 내 작업은 모두 완료되거나, 모두 취소되어야 한다.
    C(Consisitency) 일관성 : 트랜잭션 수행 전후에 db상태가 항상 유효한 상태(제약조건 충족)여야한다.
    I(Isolation) 독립성 : 여러 트랜잭션이 동시에 실행되도 서로 간섭없이 독립적으로 싱행되야한다.
