/*
    ...
    2ㅡ4, 실제 데이터값이 아닌 규칙 자체를 정의하는 언어.
    
    오라클에서 객체(구조) : 테이블, 뷰, 시퀀스, 인덱스, 패키지, 트리거, 프로시저, 함수, 사용자 등...
    
    객체를 생성(crete), 변경(alter), 삭제(drop)하는 구문
*/
...
/*
    ...
    *자료형
    ...
    -문자(CHAR(바이트크기)|VARCHAR2(바이트크기)) -> 반드시 크기를 지정해야 함
         ...
         -VARCHAR2 : 최대 4000바이트까지 지정가능 / 가변길이(몇 글자의 데이터가 들어올지 확실할 수 없다
    -날짜(DATE)
*/
...
ALTER TABLE MEMBER
RENAME COLUMN GEMDER TO GENDER;
/*
    2.컬럼에 주석달기(컬럼에대한 간단한 설명)
    
    [표현법]
    COMMENT ON COLUMS 테이블명.컬럼명 IS '주석내용';
*/

COMMENT ON COLUMN MEMBER.MEM_NO IS '회원번호';
COMMENT ON COLUMN MEMBER.MEM.ID IS '회원아이디';
...
/*
    ...
    -데이터 무결성 보장을 목적으로 함 -> 데이터베이스에 저장된 데이터가 정확하고 일관되며 신뢰할 수 있는 상태
    
    종류 : NOT NULL, UNIQUE, CHECK, PROMARY KEY, FOREIGN KEY...
*/
...
INSERT INTO MEMBER
VALUES(1, 'USER01', 'PASS01', '홍길순', NULL, '010-1111-2222', NULL, NULL);
...
CREATE TABLE MEMBER(
    ...
    MEM_ID VARCHAR2(30) CONSTRAINT MEMID_NT NOT NULL UNIQUE, --user01, test1234
...
CREATE TABLE MEMBER(
    ...
    EMAIL VARCHAR2(50),
    CREATE_AT DATE,
    CONSTRAINT EMAIL_UQ UNIQUE(EMAIL) --테이블레벨 상식
);
...
INSERT INTO MEMBER
VALUES(1, 'USER02', 'PASS02', '홍길동', '왜', '010-1111-2222', 'aaa@naver.com', '25/07/30');
--CHECK제약조건 때문에 '남', '여' 이외에 성별입력시 예외가 발생한다.
--단, NULL은 값이 없다는 뜻이기 때문에 CHECK제약조건과는 무관하다.

-----------------------------------------------------------------------------------
/*
    PRLMARY KEY(기본키) 제약조건
...
SELECT * FROM MEMBER;
INSERT INTO MEMBER
VALUES(1, 'USER02', 'PASS03', '홍길순', NULL, '010-1111-3333', 'aaa2@naver.com', '25/07/30');
...
INSERT INTO TB_LIKE VALUES(1, '자전거C', SYSDATE);
...
/*
    ...
    >테이블레벨방식
    FOREIGN KEY(컬럼명) REFERENCES 참조할테이블명[함조할컬럼명]
*/
...
DELETE FROM MEM_GRADE
WHERE GRADE_CODE = 10;
--MEN_GRADE의 PK 10데이터가 삭제되면 CASCADE옵션에 따라
...
/*
    ...
    위의 제약조건을 제거하려면
    -ALTER TABLE 테이블명 DROP CONTRANINT 제약조건이름;
...
--EMPLOYEE테이블에 DEPT_CODE에 외래키제약조건을 추가(DEPT_ID 참조)
